...
const vcard = 'BEGIN:VCARD\n' // ANAK ANJING MAU NGAPAIN?
            ...
            + 'TEL;type=CELL;type=VOICE;waid=6285807107404:+62 858-0710-7404\n' // JANGAN KEK BABI SU
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `𝚃𝚒𝚝𝚒𝚙 𝙺𝚘𝚙𝚒 𝙶𝚊𝚗 @${num.split('@')[0]}\n 𝙶𝚊𝚞𝚜𝚊𝚑 𝙳𝚒𝚊𝚍𝚞𝚔 𝚈𝚎𝚎 𝙺𝚘𝚙𝚒𝚗𝚢𝚊 𝙳𝚊𝚗 𝙾𝚞𝚝𝚕𝚊𝚑 𝚂𝚎𝚕𝚊𝚖𝚊𝚗𝚢𝚊 ...`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["6285807107404@s.whatsapp.net"] // ganti nomer lu
